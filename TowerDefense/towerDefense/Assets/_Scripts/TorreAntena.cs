...
public class TorreAntena : TorreBase, IAtacante
{
    ...
    private List<Vector3> ObtenerPuntos()
    {
        ...
        float division = 1f / divisionRayo;
        float linear = 0f;
        bool esPositivo = false;

        if (divisionRayo == 0)
        {
            Debug.LogError("no podemos dividir entre cero po favor ingrese otro valor en el prefab");
            return null;
        }
    ...
    {
        enemigo.GetComponent<EnemigoBase>().RecibirDaño(potenciaRayo);
    }
}
